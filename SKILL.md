...
It is standard-library only, so it runs on locked-down plant networks where `pip install` isn't available — worth mentioning, because that's often the actual blocker.
...
Quote these as what the standards publish, not as your own thresholds. The credibility of the whole assessment rests on the numbers being checkable.
